    ...
    public static void addToQueue(passenger[] pump1, passenger[] pump2, passenger[] pump3, passenger[] pump4, passenger[] pump5, int[] fuelStock) {
       //Method to add the customers according to the minimum empty spaces
        ...
        if (Queue_no == 1) {
            ...
            if (num == 6) {
                if(!FuelQueue.waitingList.isFull()){
                    System.out.println("oops!! All the Queues are full, you will be added to the waiting List");
                    passenger passenger= new passenger();
                    passenger.Name("empty");
                    passenger.surname("empty");
                    passenger.setVehicle_no();
                    passenger.setNo_liters();
                    FuelQueue.waitingList.enQueue(passenger);
                }else {
                    ...
                    for (int i = 0; i < 6; i++) {
                        if ((position + i + 1) == 5) {
                            ...
                            if(!waitingList.isEmpty()){
                                passenger fromWaitingList = waitingList.deQueue();
